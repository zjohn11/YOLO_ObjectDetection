"""
Dataset preparation script for YOLO Object Detection Project.

This script downloads, filters, augments, and organizes datasets
...
def get_filtered_dataset(class_name, target_count, max_people=1):
    """
    Filters a FiftyOne Zoo dataset to retain only samples containing the target class.

    Args:
        class_name (str): The class name to filter (e.g., "Dog", "Cat").
        target_count (int): The number of filtered samples desired.
        max_people (int, optional): Maximum allowed people per image when filtering "Person". Defaults to 1.

    Returns:
        fiftyone.core.dataset.Dataset:
            A FiftyOne dataset containing exactly `target_count` samples,
            each of which has at least one Detection with label `class_name`.
    """
    filtered_samples = []
    attempt = 1
    ...
    while len(filtered_samples) < target_count:
        ...
        for sample in raw_ds:
            ...
            if filtered:
                sample["ground_truth"] = Detections(detections=filtered)
...
def augment_images(input_dir, n_target):
    """
    Augments images from a given directory to reach a specified target count.

    Args:
        input_dir (str): Directory containing original images.
        n_target (int): Desired total number of images after augmentation.
    ...
    while save_count < n_needed:
        img_path = os.path.join(input_dir, existing_images[i % n_existing])
        ...
        cv2.imwrite(os.path.join(input_dir, new_filename), augmented)
        save_count += 1
        i += 1
...
if __name__ == "__main__":
    ...
    existing_images = [
        f for f in os.listdir(lightsaber_dir)
        if f.lower().endswith(('.jpg', '.png'))
    ]
    n_existing = len(existing_images)
    n_target = 1000
    ...
    existing_images = [
        f for f in os.listdir(dalek_dir)
        if f.lower().endswith(('.jpg', '.png'))
    ]
    n_existing = len(existing_images)
    n_target = 1000
    ...
    for sample in lightsaber_ds:
        detection = Detection(label="Lightsaber", bounding_box=[0.0, 0.0, 1.0, 1.0])
        sample["ground_truth"] = Detections(detections=[detection])
        sample.save()
    ...
    for sample in dalek_ds:
        detection = Detection(label="Dalek", bounding_box=[0.0, 0.0, 1.0, 1.0])
        sample["ground_truth"] = Detections(detections=[detection])
        sample.save()
    ...
    # Add samples from each datasets
    for ds in datasets:
