...
def get_filtered_dataset(class_name, target_count, batch_size=600):
    filtered_samples = []
    total_loaded = 0
    attempt = 1
...
dog_ds = get_filtered_dataset('Dog', 600)
dog_ds.default_classes = class_list
# Load cat images
cat_ds = get_filtered_dataset('Cat', 600)
cat_ds.default_classes = class_list
# load 600 human images
human_ds = get_filtered_dataset('Person', 600)
...
lightsaber_crawler.crawl(keyword='lightsaber', max_num=200, file_idx_offset=0)

# Download Dalek images
dalek_crawler = GoogleImageCrawler(storage={'root_dir': './dalek_images'})
dalek_crawler.crawl(keyword='dalek', max_num=200, file_idx_offset=0)
